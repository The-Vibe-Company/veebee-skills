...
Options are two to five words, in the user's language, and hold nothing but the option. No "(exclusifs)", no parenthesis bolted onto the question. The skill's opinion lives in a 💡 suggestion, as in `eureka`: almost every question carries one, in the first person, with the reason in one line, and the reason names what points there (something the user said, a fact found and linked, what the option commands downstream). With the question tool the suggested option goes first with "(Recommandé)" on its label and the reason as its description; in the chat the 💡 line closes the options. No suggestion on the final check, on the offer to go deep, or on a question of taste. The sizes are said as *une seule chose*, *plusieurs morceaux*, *plusieurs produits*; `sketch`, `product` and `platform` are internal names for the documents, never spoken. No calendar words in any question.
