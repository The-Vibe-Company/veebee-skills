...
- `/eureka`
- Model-invoked on blank-page phrases only: "je cherche une idée", "j'ai pas d'idée", "trouve-moi un projet", "I need an idea", "what should I build". Never on "I have a rough idea": that is `define`'s territory.

## Conversation

- First turn opens with two or three sentences on what Eureka does and what it is for (one Idea by interview, a canvas that fills as the user answers, `.veebee/idea.md` at the end, ready for `define`), then always the same question: where are you with your idea?
- Three branches:
  ...
  - **An idea already clear**: put it into the frame and point to `define` without digging.
...
- Ends when the user confirms. Writes the file, updates the canvas one last time, says one sentence: the Idea is in `.veebee/idea.md` and the next step is `/define`. Never launches another skill.
